...
const FeatureCard = ({ title, isNew, description, icon, iconBg }: Props) => {
  const Icon = icon;
  return (
    <section className="flex items-center gap-4 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition">
      {/* Icon */}
      <div
        className={`rounded-xl size-12 shrink-0 flex items-center justify-center ${iconBg}`}
      >
      ...
      {/* Action Button */}
      <button className="px-3 py-1.5 text-sm bg-gray-100 dark:bg-gray-700 dark:text-gray-100 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition cursor-pointer">
        Open
      </button>
    </section>
  );
